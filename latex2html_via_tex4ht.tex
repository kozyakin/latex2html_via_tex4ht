...
\begin{figure}[!hbt]
  ...
  auto,>=latex',node font=\small]
    ...
    \begin{scope}[xscale=0.5, yscale=2.0]
      ...
      \path (SS) node[color=blue,left] {$H_{1}S$};
    \end{scope}
    %
    \begin{scope}[xscale=-0.5, yscale=-2.0]
      \pgfmathsetmacro{\s}{2}
      ...
      \coordinate (SS) at ({cos(\a)*\s},{sin(\a)*\s});
      \draw[fill=blue!10, line width=0.3pt] (SS) to (0,0) to (S) arc [start angle=-\a, end angle=\a, radius=\s];
      \path (SS) node[color=blue,below] {$H_{1}\tilde{S}$};
    \end{scope}
    %
    \pgfmathsetmacro{\s}{2}
    \coordinate (S) at ({cos(\a)*\s},{sin(-\a)*\s});
    ...
    \path (SS) node[color=blue,below left,xshift=0.66ex] {$S$};
    %
    \begin{scope}[scale=-1.0]
      ...
      \coordinate (SS) at ({cos(\a)*\s},{sin(\a)*\s});
      \draw[fill=blue!5, line width=0.3pt] (SS) to (0,0) to (S) arc [start angle=-\a, end angle=\a, radius=\s];
      \path (SS) node[color=blue,above right,xshift=-0.66ex] {$\tilde{S}$};
    \end{scope}
    %
    \begin{scope}[xscale=0.5, yscale=2.0]
      \pgfmathsetmacro{\s}{2}
